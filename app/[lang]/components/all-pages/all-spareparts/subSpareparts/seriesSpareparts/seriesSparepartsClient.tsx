...
import getAllSparepartBySeries from '@/app/actions/get-all-spareparts-by-series';
import AllDriversandFiltersProducts from '../../../components-all-product/all-filters';

type Props = {
  params: { sparepartSubCategory?: string, sparepartSeries?: string }
}
...
export default function SparepartBySeriesPage(props: Props) {
  ...
  useEffect( () => {
    async function fetchData(){
      try{
        const { sparepartSubCategory = '',  sparepartSeries = '' } = await props.params;
        let tempData = await getAllSparepartBySeries(sparepartSubCategory, sparepartSeries);
