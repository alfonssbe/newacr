import { AllCategory, AllProductsForHome, CachedAllProducts, Products, Size, Specifications, SubCategoryFilters } from "../types";
import { redirect } from "next/navigation";

const API=`${process.env.NEXT_PUBLIC_ROOT_URL}/${process.env.NEXT_PUBLIC_FETCH_ALL_SPAREPARTS_BY_SUB_SUB_CATEGORY}`;

const getAllSparepartBySubSubCategory = async (subcategory: string, series: string, subsubcategory: string): Promise<CachedAllProducts> => {
  ...
  const API_EDITED_FIRST = API.replace('{sparepartSubCategory}', subcategory)
  const API_EDITED_SECOND = API_EDITED_FIRST.replace('{sparepartSeries}', series)
  const API_EDITED = API_EDITED_SECOND.replace('{sparepartSubSubCategory}', subsubcategory)
...
export default getAllSparepartBySubSubCategory;
