...
export async function DELETE(
  ...
  try {
    ...
    const size_deleted = await prismadb.size.deleteMany({
      where: {
        id: params.sizeId
      }
    });
    ...
    product && product.length > 0 && product.map((prod) => {
      revalidatePath(`/produk/${prod.slug}`)
      revalidatePath(`/en/products/${prod.slug}`)
    })
  
    return NextResponse.json(size_deleted);
  } catch (error) {
