...
export const CellAction: React.FC<CellActionProps> = ({
  ...
  const onConfirm = async () => {
    try {
      ...
      const API_EDITED2 = API_EDITED.replace('{seriesId}', data.id)
      const response = await axios.delete(API_EDITED2);
      if(response.data === 'stillused'){
        window.location.reload()
        toast.error('Make sure you removed all products using this series first.');
      }
  ...
  const onCopy = (id: string) => {
    ...
    toast.success('Series ID copied to clipboard.');
  }
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() => router.push(`${process.env.NEXT_PUBLIC_ADMIN_FOLDER_URL}/${params.brandId}/series/${data.id}`)}
          >
